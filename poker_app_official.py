...
class TransformerBot:
    ...
    def _load_transformer_model_with_fallback(self, preferred_ckpt_path: str) -> bool:
        candidates = [preferred_ckpt_path]
        if DEFAULT_CKPT not in candidates:
            candidates.append(DEFAULT_CKPT)
        if "augment_poker_transformer.pt" not in candidates:
            candidates.append("augment_poker_transformer.pt")
